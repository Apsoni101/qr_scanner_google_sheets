import 'package:dartz/dartz.dart';
import 'package:qr_scanner_practice/core/services/network/failure.dart';
import 'package:qr_scanner_practice/feature/history/data/data_source/scans_history_remote_data_source.dart';
import 'package:qr_scanner_practice/feature/history/domain/repo/scans_history_remote_repository.dart';
import 'package:qr_scanner_practice/feature/result_scan/domain/entity/pending_sync_entity.dart';

class ScansHistoryRemoteRepositoryImpl implements ScansHistoryRemoteRepository {
  ScansHistoryRemoteRepositoryImpl({required this.remoteDataSource});

  final ScansHistoryRemoteDataSource remoteDataSource;

  @override
  Future<Either<Failure, List<PendingSyncEntity>>> getAllScansFromAllSheets() =>
      remoteDataSource.getAllScansFromAllSheets();
}
